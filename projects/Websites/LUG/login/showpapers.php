...
<h3><a onclick="showpage('login/home.php');">My Home</a> : Papers uploaded by me</h3>
<br>
<h2>Papers uploaded by me ...</h2>
<br>
<?php
...
{
echo "<b>Nothing uploaded yet. Please upload a paper.<b>";
mysql_close();
return;
}
...
echo '
	...
	<tr>
		...
		<a onclick=showpage("login/paper.php?no=' . $row['no'] . '")>'. $topic .'</a> | '. $row['format'] .'
		<br>
		'.$row["catch"].'
		</td>
		...
		<table border="0" cellpadding="0" style="border-collapse: collapse" width="100%" id="table2">
			<tr>
				<td align="left">
				<a onclick=showpage("login/paper.php?no=' . $row['no'] . '")>View</a>
				</td>
				<td align="left">
				<a onclick=showpage("login/deletepaper.php?no=' . $row['no'] . '")>Delete</a>
				</td>
			</tr>
		</table>
		</td>
