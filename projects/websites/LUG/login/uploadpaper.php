...
/////////////////////////////////// now decide the fields ///////////////////
	$max = "";
	$f=fopen("./papermax","r+") or exit("Unable to open file for reading!");
	while (!feof($f))
	{
		$x=fgetc($f);
		$max = $max . $x;
	...
	$max = $max + 1;
	$no = padzero($max, 5);
	$f=fopen("./papermax","w") or exit("Unable to open file for writing!");
	fwrite($f, $max);
	fclose($f);
	...
	$date = date('d M Y');
	$format = $_POST["format"];
	$filename = getfilename($_FILES['uploadpaper']['name'], $no, $format);
...
if(move_uploaded_file($_FILES['uploadpaper']['tmp_name'], $target_path))
{
	...
	$query = 'INSERT INTO papers VALUES("'.$no.'","0","'.$topic.'","'.$catch.'","'.$username.'","'.$date.'","'.$filename.'","'.$format.'","0")';
	$result = mysql_query($query);
	mysql_close();

	// all work done now redirect to a page giving out the informations
	header('Location:../?page=login/uploadfine.php&file=' . $_FILES['uploadpaper']['name'] . '&type=' . $_FILES['uploadpaper']['type'] . '&size=' . $_FILES['uploadpaper']['size']);
}
else
{
	header('Location:../?page=login/uploadfine.php&error=' . $_FILES['uploadpaper']['error']);
}
